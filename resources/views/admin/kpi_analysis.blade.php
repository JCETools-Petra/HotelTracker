...
<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            {{-- Analisis Detail per Kategori Pendapatan (Diambil dari artifact kpi_analysis_blade_mom_enhancements) --}}
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div class="p-4 border dark:border-gray-700 rounded-lg space-y-2">
                        ...
                        @if (!empty($multiMonthCategoryGrowth))
                            @foreach ($multiMonthCategoryGrowth as $monthName => $growthData)
                                    ...
                                    @if(!empty($growthData))
                                        <ul class="space-y-1 text-sm">
                                            @foreach ($growthData as $data)
                                                <li class="flex justify-between {{ str_contains($data['growth'], '+') || $data['growth'] === 'Baru' ? 'text-green-600 dark:text-green-400' : (str_contains($data['growth'], '-') ? 'text-red-600 dark:text-red-400' : 'text-gray-600 dark:text-gray-400') }}">
                                                    <span class="font-medium text-gray-800 dark:text-gray-300">{{ $data['category'] }}:</span>
                                                    <span class="{{ str_contains($data['growth'], '+') || $data['growth'] === 'Baru' ? 'font-semibold' : (str_contains($data['growth'], '-') ? 'font-semibold' : '') }}">
                                                        {{ $data['growth'] }}
                                                    </span>
                                                </li>
                                                <li class="flex justify-between text-xs text-gray-400 dark:text-gray-500 pl-4 mb-1">
                                                    <span>(Saat Ini: {{ number_format($data['current_value'], 0, ',', '.') }};</span>
                                                    <span>Sebelumnya: {{ number_format($data['previous_value'], 0, ',', '.') }})</span>
                                                </li>
                                            @endforeach
                                        </ul>
                                    @else

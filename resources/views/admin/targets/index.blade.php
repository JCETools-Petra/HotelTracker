                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse ($targets as $target)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $target->month_name }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                            ...
                                            <form method="POST" action="{{ route('admin.targets.destroy', $target->id) }}" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus target untuk properti \'{{ addslashes($target->property->name ?? 'N/A') }}\' periode {{ $target->year }}-{{ $target->month_name }}? Tindakan ini tidak dapat batalkan.');">
                                                @csrf
                                                @method('DELETE')
